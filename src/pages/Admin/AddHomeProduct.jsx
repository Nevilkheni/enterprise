...
const AddHomeProduct = ({ fetchProducts, setError }) => {
  ...
  // ✅ Check duplicate in both showcaseCards & allProducts
  const checkDuplicate = async (name) => {
    try {
      ...
      const [showcaseSnap, productsSnap] = await Promise.all([
        getDocs(showcaseQ),
        getDocs(productsQ),
      ]);

      return !showcaseSnap.empty || !productsSnap.empty;
    } catch (error) {
      console.error("Error checking duplicate:", error);
      return false;
    }
  ...
  const handleHomeProductSubmit = async (e) => {
    ...
    try {
      // ✅ duplicate check
      const isDuplicate = await checkDuplicate(homeProductForm.name);
      if (isDuplicate) {
        setDuplicateWarning(
          `⚠️ A product with the name "${homeProductForm.name}" already exists!`
        );
        setUploading(false);
        return;
      }
      ...
      const existingCards = await getDocs(collection(db, "showcaseCards"));
      const existingInPosition = existingCards.docs.find(
        (docSnap) => docSnap.data().position === homeProductForm.position
      );

      if (existingInPosition) {
        const existingName = existingInPosition.data().title;
        if (
          !window.confirm(
            `Position ${homeProductForm.position} already has "${existingName}". Replace it?`
          )
        ) {
          setUploading(false);
          return;
        ...
        await deleteDoc(doc(db, "showcaseCards", existingInPosition.id));
        alert(
          `Removed existing product "${existingName}" from position ${homeProductForm.position}`
        );
      }
      ...
      // ✅ add to showcaseCards
      await addDoc(collection(db, "showcaseCards"), productData);

      // ✅ also add to allProducts
      await addDoc(collection(db, "allProducts"), {
        name: homeProductForm.name,
        description: `Material: ${homeProductForm.material}, Thickness: ${homeProductForm.thickness}, Length: ${homeProductForm.length}`,
        image: imageUrl,
